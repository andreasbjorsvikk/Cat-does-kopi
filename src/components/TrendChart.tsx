...
const TrendChart = ({ sessions, period, month, year, metric, chartType = 'bar', selectedTypes }: TrendChartProps) => {
  ...
  // Fetch Apple Health steps for the visible range. Web returns empty.
  const stepsRange = useMemo(() => {
    if (period === 'month') {
      const last = new Date(year, month + 1, 0).getDate();
      return {
        start: `${year}-${String(month + 1).padStart(2, '0')}-01`,
        end: `${year}-${String(month + 1).padStart(2, '0')}-${String(last).padStart(2, '0')}`,
      };
    }
    if (period === 'year') return { start: `${year}-01-01`, end: `${year}-12-31` };
    return { start: '2015-01-01', end: '2099-12-31' };
  }, [period, month, year]);
  const { stepsByDate, available: stepsAvailable } = useDailySteps(stepsRange.start, stepsRange.end);
  ...
  const typeOrder = useMemo(() => {
    if (isSteps) return ['steps' as SessionType];
  ...
  }, [sessions, metric, isSteps]);

  const typeColors = useMemo(() => {
    const colors: Record<string, string> = {};
    if (isSteps) {
      colors['steps'] = 'hsl(210, 90%, 55%)';
      return colors;
    }
    ...
    return colors;
  }, [typeOrder, isDark, isSteps]);

  const data = useMemo(() => {
    // Steps series is sourced from Apple Health, not from workout sessions.
    if (isSteps) {
      const sumRange = (predicate: (date: string) => boolean) => {
        let sum = 0;
        stepsByDate.forEach((v, date) => { if (predicate(date)) sum += v; });
        return sum;
      };
      if (period === 'month') {
        ...
        return Array.from({ length: daysInMonth }, (_, i) => {
          ...
          const val = stepsByDate.get(dateStr) || 0;
          return { label: day.toString(), steps: val, _total: val };
        });
      }
      if (period === 'year') {
        return Array.from({ length: 12 }, (_, i) => {
          const monthStr = `${year}-${String(i + 1).padStart(2, '0')}`;
          const val = sumRange(d => d.startsWith(monthStr));
          return { label: monthLabels[i], steps: val, _total: val };
        });
      }
      const years = new Set<number>();
      stepsByDate.forEach((_, d) => years.add(parseInt(d.slice(0, 4), 10)));
      if (years.size === 0) return [];
      const minYear = Math.min(...years);
      const maxYear = Math.max(...years);
      return Array.from({ length: maxYear - minYear + 1 }, (_, i) => {
        const y = minYear + i;
        const val = sumRange(d => d.startsWith(String(y)));
        return { label: y.toString(), steps: val, _total: val };
      });
    }
  ...
  }, [sessions, period, month, year, metric, typeOrder, isSteps, stepsByDate]);
        ...
        {items.map((item: any) => {
          ...
          return (
            <div key={item.dataKey} className="flex items-center gap-2.5 py-0.5">
              ...
              <span className="text-muted-foreground">{isTotal ? 'Totalt' : (item.dataKey === 'steps' ? 'Skritt' : sessionTypeConfig[item.dataKey as SessionType]?.label)}</span>
    ...
    <div className="rounded-2xl p-4 flex flex-col h-full relative overflow-hidden bg-gradient-to-br from-card/90 to-card/60 backdrop-blur-xl border border-border/30 shadow-[0_4px_24px_rgba(0,0,0,0.06)]">
      ...
      {!hasData && (
        <div className="absolute inset-0 flex items-center justify-center bg-background/30 backdrop-blur-[2px] rounded-2xl">
          <p className="text-sm text-muted-foreground font-medium text-center px-6">
            {isSteps && !stepsAvailable
              ? 'Apple Health er kun tilgjengelig i iOS-appen.'
              : isSteps
                ? 'Ingen skrittdata for denne perioden.'
