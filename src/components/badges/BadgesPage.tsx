...
const BadgesPage = () => {
  ...
  // Listen for scroll-to-badge event
  useEffect(() => {
    const handler = (e: Event) => {
      const badgeId = (e as CustomEvent)?.detail?.badgeId;
      if (!badgeId || badges.length === 0) return;
      const targetBadge = badges.find(b => b.badge.id === badgeId);
      if (targetBadge) {
        setFilter(targetBadge.badge.category);
        setTimeout(() => {
    ...
    window.addEventListener('scroll-to-badge', handler);
    return () => window.removeEventListener('scroll-to-badge', handler);
  }, [badges]);
              ...
              <div className="grid grid-cols-3 gap-3">
                {badgeList.map(b => {
                  ...
                  const subLabel = isStreak
                    ...
                    : '';
                  return (
                  <div key={b.badge.id} data-badge-id={b.badge.id} className="relative flex flex-col items-center gap-1.5">
