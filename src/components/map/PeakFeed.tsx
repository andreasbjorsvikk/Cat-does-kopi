...
const PeakFeed = () => {
  ...
  const loadFeed = async () => {
    ...
    try {
      ...
      // Privacy is enforced server-side via RLS on peak_checkins; we only need
      // public profile fields for display. Fetch friends + self + their children.
      ...
      const visibleFriendIds = profiles.map(p => p.id);
      const visibleChildParentIds = profiles.map(p => p.id);
