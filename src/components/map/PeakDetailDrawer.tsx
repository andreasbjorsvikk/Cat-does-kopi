...
import { Mountain, MapPin, Check, Loader2, Pencil, Trash2, CalendarIcon, UserPlus, X, Search, List, Users, ImageIcon, Sunrise, Sunset, Info, Trophy, CloudSun } from 'lucide-react';
...
interface PeakDetailDrawerProps {
  ...
  onCreateRoute?: (peak: Peak, source: 'gps' | 'pick') => void;
}
...
const PeakDetailDrawer = ({ peak, open, onClose, checkins, onCheckinSuccess, adminMode, onEdit, onDelete, onShowRoute, onHideRoute, isRouteShown, fromTopperTab, onShowOnMap, onCreateRoute }: PeakDetailDrawerProps) => {
  const { user } = useAuth();
    ...
    <>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full grid grid-cols-3">
                ...
                <TabsTrigger value="plan" className="text-sm gap-1.5">
                  <CloudSun className="w-5 h-5" /> <span className="text-xs">{language === 'no' ? 'Vær' : 'Weather'}</span>
                </TabsTrigger>
              </TabsList>
              ...
              <div className="space-y-4 mt-3" role="tabpanel" aria-label="Info">
                ...
                <div className="flex flex-col gap-2">
                  {peak.route_status === 'approved' && onShowRoute && (
                    isRouteShown ? (
                      ...
                      <Button variant="outline" size="sm" className="w-full" onClick={() => onShowRoute(peak, fromTopperTab)}>
                        {language === 'no' ? 'Vis rute' : 'Show route'}
                      </Button>
                    )
                  )}
                  {onCreateRoute && (
                 ...
                 {/* Route stats boxes */}
                 {peak.route_status === 'approved' && (peak.route_distance_m || peak.route_duration_s) && (
                   <div className="grid grid-cols-3 gap-2">
                     <div className="flex flex-col items-center gap-0.5 p-2 rounded-lg bg-muted/30 border border-border/30">
                       <p className="text-[10px] text-muted-foreground">{language === 'no' ? 'Distanse' : 'Distance'}</p>
                       <p className="text-sm font-semibold">{((peak.route_distance_m || 0) / 1000).toFixed(1)} km</p>
                     </div>
                     <div className="flex flex-col items-center gap-0.5 p-2 rounded-lg bg-muted/30 border border-border/30">
                     ...
                     <div className="flex flex-col items-center gap-0.5 p-2 rounded-lg bg-muted/30 border border-border/30">
                       ...
                       <p className="text-sm font-semibold">{Math.round((peak.route_duration_s || 0) / 60)} min</p>
                     </div>
                   </div>
                 )}
      ...
      {peak && onCreateRoute && (
        <CreateRouteDialog
          ...
          onFromGps={() => { onClose(); onCreateRoute(peak, 'gps'); }}
          onPickOnMap={() => { onClose(); onCreateRoute(peak, 'pick'); }}
          language={language}
        />
      )}
    </>
