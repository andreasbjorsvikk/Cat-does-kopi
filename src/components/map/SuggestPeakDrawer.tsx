...
const SuggestPeakDrawer = ({ open, onClose, latitude, longitude }: SuggestPeakDrawerProps) => {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true);
    try {
      // Compute distance from user to the suggested peak client-side so we don't
      // store the submitter's raw GPS coordinates server-side.
      let submissionDistanceM: number | null = null;
      if (userLocation) {
        const R = 6371000;
        const toRad = (d: number) => (d * Math.PI) / 180;
        const dLat = toRad(userLocation.lat - latitude);
        const dLng = toRad(userLocation.lng - longitude);
        const a =
          Math.sin(dLat / 2) ** 2 +
          Math.cos(toRad(latitude)) *
            Math.cos(toRad(userLocation.lat)) *
            Math.sin(dLng / 2) ** 2;
        submissionDistanceM = 2 * R * Math.asin(Math.sqrt(a));
      }
      await submitSuggestion({
        ...
        latitude,
        longitude,
        submission_distance_m: submissionDistanceM,
      });
