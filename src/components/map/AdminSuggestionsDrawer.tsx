  ...
  const handleReview = async (status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      if (status === 'approved') {
        ...
        let county = '';
        try {
          const res = await fetch(`https://api.kartverket.no/kommuneinfo/v1/punkt?nord=${selected.latitude}&ost=${selected.longitude}&koordsys=4258`);
          const data = await res.json();
          municipality = data.kommunenavn || '';
          county = data.fylkesnavn || '';
        } catch (e) {}

        const resolvedArea = municipality && county ? `${municipality}, ${county}` : '';
        ...
        let dist = 0;
        if (selected.submission_distance_m != null) {
          dist = selected.submission_distance_m;
          if (dist <= 100) {
            try {
