...
const WorkoutDialog = ({ open, onClose, onSave, session, defaultDate }: WorkoutDialogProps) => {
  ...
  useEffect(() => {
    if (open) {
      const sessionType = session?.type || settings.defaultSessionType;
      setType(sessionType);
      ...
      // For trappemaskin edits: convert stored meters back to floors
      if (session && sessionType === 'trappemaskin' && session.elevationGain) {
        setElevationMode('floors');
        setElevationGain(Math.round(session.elevationGain / 3).toString());
      } else {
        setElevationMode('meters');
