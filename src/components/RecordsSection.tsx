...
const RecordsSection = () => {
  ...
  // Listen for external requests to switch to a specific record sub-tab
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<{ tab?: RecordTab }>).detail;
      if (detail?.tab && ['running', 'cycling', 'hiking'].includes(detail.tab)) {
        setTab(detail.tab);
      }
    };
    window.addEventListener('set-records-tab', handler as EventListener);
    // Consume pending target set before component mounted
    const pending = (window as any).__pendingRecordsTab as RecordTab | undefined;
    if (pending) {
      setTab(pending);
      delete (window as any).__pendingRecordsTab;
    }
    return () => window.removeEventListener('set-records-tab', handler as EventListener);
  }, []);
