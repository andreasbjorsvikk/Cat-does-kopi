...
export const stravaService = {
  ...
  async sync(): Promise<{ synced: number; total: number }> {
    ...
    if (!res.ok) {
      ...
      if (body.includes('Inactive') || body.includes('Application')) {
        throw new Error('Strava-appen er deaktivert av Strava. Gå til strava.com/settings/api for å reaktivere den.');
      }
      throw new Error(body || 'Failed to sync');
    }
    return res.json();
  },
