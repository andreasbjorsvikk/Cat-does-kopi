...
export async function checkinPeak(userId: string, peakId: string, checkedInAt?: string, imageFile?: File | null, checkedInBy?: string, isOffline?: boolean): Promise<PeakCheckin> {
  ...
  if (imageFile) {
    ...
    payload.image_url = await createPeakImageSignedUrl(path);
  }
...
async function createPeakImageSignedUrl(path: string): Promise<string> {
  const { data, error } = await supabase.storage
    .from('peak-images')
    .createSignedUrl(path, PEAK_IMAGE_URL_TTL_SECONDS);
  if (error || !data) throw error ?? new Error('Failed to sign peak image URL');
...
export async function updateCheckinImage(checkinId: string, userId: string, imageFile: File): Promise<string> {
  ...
  const { error: uploadErr } = await supabase.storage
    ...
    .upload(path, imageFile, { contentType: imageFile.type, upsert: false });
  if (uploadErr) throw uploadErr;
  const imageUrl = await createPeakImageSignedUrl(path);
