...
export async function getChallengeProgress(challenge: ChallengeRow, participantUserIds: string[]) {
  ...
  // Use security-definer RPC so participants always see each other's progress
  // regardless of individual workout privacy settings.
  const { data, error } = await supabase.rpc('get_challenge_progress', { _challenge_id: challenge.id });
  if (error) {
    console.error('get_challenge_progress error:', error);
    return progress;
  }

  for (const row of (data || []) as { user_id: string; value: number | string }[]) {
    progress[row.user_id] = Number(row.value) || 0;
  }
  return progress;
}
