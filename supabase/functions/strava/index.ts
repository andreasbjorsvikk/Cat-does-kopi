...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "sync") {
      ...
      let after: number;
      if (afterParam) {
        after = parseInt(afterParam);
      } else {
        // Anchor on the user's most recent Strava activity (minus 1 day buffer) so we
        // never miss newly added activities when the user has >500 recent activities
        // (Strava's `after` filter returns results in ASCENDING order, so paginating
        // from 180 days ago would page through the OLDEST activities first).
        const { data: latest } = await admin.from("workout_sessions")
          .select("date")
          .eq("user_id", userId)
          ...
          .maybeSingle();
        if (latest?.date) {
          after = Math.floor(new Date(latest.date).getTime() / 1000) - 86400;
        } else {
          after = Math.floor(Date.now() / 1000) - 180 * 86400;
        }
      }
      ...
      while (page <= 10) {
        ...
        if (!res.ok) {
          stravaError = `Strava API ${res.status}: ${await res.text().catch(() => "")}`;
          console.error("Strava sync fetch error:", stravaError);
          break;
        }
      ...
      if (stravaError && activities.length === 0) {
        return new Response(JSON.stringify({ error: stravaError }), { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } });
      }
